...
extension KakaotalkViewController: ButtonDelegate {
...
extension KakaotalkViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func openLibrary() {
        ...
        if let image = info[UIImagePickerController.InfoKey.originalImage] as? UIImage,
            ...
            UploadService.shared.uploadImage(token, image, url.lastPathComponent) { networkResult in
                ...
                case .success(let profileData):
                    ...
                    guard let profileCell = self.katalkTableView.cellForRow(at: IndexPath(row: 0, section: 0)) as? Conv1 else { return }
                    profileCell.buttonOutlet.setBackgroundImage(image, for: .normal)
